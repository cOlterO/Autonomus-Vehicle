...
const int distance_to_walls = 22;  // How many cm appart should the sensors read to be centered
// |                   |
// |       |Car|       |
// |-------|   |-------|
...
void loop() {
  ...
  if (d2 < distance_to_walls) {          // Left side is getting closer to wall
    pwmEnB += (distance_to_walls - d2);  // Right motor increases speed
    forward(pwmEnA, pwmEnB);
  }

  if (d3 < distance_to_walls) {          // Right side is getting closer to wall
    pwmEnA += (distance_to_walls - d3);  // Left motor increases speed
    forward(pwmEnA, pwmEnB);
  }
}
